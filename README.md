...
## Описание
___
Данный сервис предоставляет поиск ближайших для перевозки грузов. Вы можете добавлять грузы, отслеживать сколько автомобилей рядом с грузом (подходят по грузоподъемности и в пределах обозначенного расстояния (в милях)), для конкретного груза отслеживать точное расстояние до каждого автомобиля, обновлять информацию о грузах и автомобилях, удалять грузы. Так же локация каждого автомобиля автоматически *обновляется каждые 3 минуты.
